...
- 回帰テスト：**1129 passed / 7 skipped**（独立リファレンスツール27件を含む）。合成テストに加え、固定55個のネイティブITL構造と50個の対応COM記録を検査しました。スキップはCOM記録がない5件と、Linux検証環境に `win32gui` がないWindows専用helper 2件です。テスト件数やスキップ件数はネイティブ操作の回数ではありません。
