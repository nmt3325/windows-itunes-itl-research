...
- [`corpus-manifest.json`](corpus-manifest.json) hashes all 252 retained ITLs and links the nested reference, fresh-native, path/time, smart-playlist, reference-generation, and media-field analyses. The four checked-in one-track/three-track reference fixtures are `verified` only for their pinned hashes.
...
- A media-backed follow-up passed fresh native `AddFile` initialization and a separate baseline restart for both failed fields, then reproduced the exact `Lyrics` COM exception and `Enabled=false`→`true` immediate readback. Its first run is retained as a pre-mutation harness-identity failure, not native rejection; the corrected v2 run is the canonical bounded negative. See [`evidence/native/media-field-followup-20260922-v2/README.md`](evidence/native/media-field-followup-20260922-v2/README.md).
