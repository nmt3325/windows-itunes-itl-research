...
U-17 remains open. Line-transition guidance is coarse and not branch-complete, and neither parser campaign proves safety or native acceptance. The filesystem audit is one Linux scratch-filesystem campaign with deterministic scheduling/fault injection; it does not establish hostile-directory safety, process-crash or power-loss durability, network/unusual-filesystem behavior, alternate-runtime behavior, or Windows parity. Closure still requires broader parser coverage/fault campaigns plus real crash/power-loss and broader filesystem matrices. Those results still would not prove native acceptance.
