...
U-17 remains open. Line-transition guidance is coarse and not branch-complete, and neither parser campaign proves safety or native acceptance. The two local-filesystem campaigns are bounded to one Linux scratch filesystem and one exact Windows Server 2025 / CPython 3.12.10 / NTFS runner qualification. They do not establish hostile-directory safety, uncontrolled-crash or power-loss durability, directory-entry durability, network/unusual-filesystem behavior, alternate-runtime behavior, or universal Windows/NTFS behavior. The two explicit `TerminateProcess` cases are neither Linux `SIGKILL` nor power-loss evidence. Closure still requires broader parser coverage/fault campaigns plus real power-loss and broader filesystem/runtime matrices. Those results still would not prove native acceptance.
