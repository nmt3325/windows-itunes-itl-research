...
U-17 remains open. Line-transition guidance is coarse and not branch-complete, and neither parser campaign proves safety or native acceptance. The filesystem evidence is limited to selected schedules on one recorded Linux runtime and local ext4 filesystem. It does not establish power-loss or directory-entry durability, hostile-directory safety, arbitrary process-crash schedules, network/unusual-filesystem behavior, alternate-runtime behavior, Windows parity, or universal atomicity. Closure still requires broader parser coverage/fault campaigns, real power-loss work, and broader process-crash/filesystem/runtime matrices. Those results still would not prove native acceptance.
