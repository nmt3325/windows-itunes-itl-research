...
A long Comment can exist on disk while the tested COM setter/read path truncates around 255 characters. In the isolated field matrix, the tested Unicode Lyrics setter raised a COM exception. Corrected fresh native WAV follow-ups reproduced the exception for that Unicode value and for the short plain-ASCII value `Plain ASCII lyrics line one` after successful save/baseline restart. The identical short ASCII property/value passed all four sessions on one deterministic MP3-backed track while iTunes rewrote the MP3. This establishes media/interface dependence for those exact cases, not a string-length boundary, generic Lyrics support, or generic rejection. Field-specific UI/COM limits, disk limits, Unicode behavior, storage authority, other media kinds, and version behavior remain unknown; U-12 remains open.
