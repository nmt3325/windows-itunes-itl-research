...
Only the named standalone Windows 12.13.10.3 profile has the full packaged semantic-write evidence. The 12.13.9.1 profile is admitted for bounded behavior without an equivalent native matrix. A separate adjacent-build cohort now shows that four exact 12.13.10.3 raw/zlib fixture hashes each passed two strict open/save/restart cycles on the exact signed iTunes 12.13.11.1 executable SHA-256 `c69e719c…dd73c`; all eight native saves independently report 12.13.11.1 and all processes exited normally without fallback. This is an exact-hash upgrade-compatibility result, not arbitrary 12.13.11.1 editing or a newly admitted production profile. The two fresh 12.13.10.3 attempts stopped on EULA/update setup modals before semantic qualification and are not ITL rejections. Other releases may change header, section, record, index, or callback semantics, and U-01 remains open.

**Closure evidence required:** a deliberately sampled set of version-pinned native fixtures, two-cycle positive and negative cases, static provenance, version-specific semantic-edit evidence, independent reproduction, and a separate matrix row for each newly admitted profile.
...
A deterministic [retained-corpus trailer census](evidence/research/20260926/retained-trailer-census/README.md) verifies and strictly parses all 400 `.itl` entries in the delivery manifest. Exactly one has a nonempty compressed trailer: the same intentionally constructed 17-byte U-13 witness; the other 399 have no trailer. The observed version distribution is 392 files reporting `12.13.10.3` and eight retained adjacent-build native saves reporting `12.13.11.1`. This curated corpus contains related snapshots, controls, generated files, and research candidates, so the observation is not a prevalence estimate and supplies no natural trailer provenance.
...
A later [separate-process hostile-directory follow-up](evidence/research/20260926/hostile-directory-process-race/README.md) runs the production writer in three fresh child processes while the parent process performs real directory replacement or parent-symlink retargeting plus random-basename spoofing at a hook-controlled pre-link boundary. The stable-parent control published the requested payload. Both adversarial scenarios made the writer return success with spoof bytes at the destination while the requested complete temporary remained in the original/displaced directory. Two fresh-root replays were byte-identical after basename normalization. This strengthens the documented hostile-parent limitation; it is three bounded schedules, not an uncontrolled race campaign or a safety proof, and performs zero power-loss/network-filesystem/native-iTunes operations.
...
A [dependency-chronology audit](evidence/research/20260926/historical-dependency-chronology/README.md) now proves that the retained Capstone/pefile entry points first appeared at `bd6944ec...` on `2026-09-09T16:07:24Z`, with no exact Capstone or pefile pin in that tree. A hash-identified PyPI JSON snapshot bounds package availability at that timestamp: 36 Capstone and 14 pefile releases had an uploaded file; Capstone 5.0.9 was the latest stable Capstone release (6.0.0a10 the latest including prereleases), and pefile 2024.8.26 was latest. This proves only availability, not installation or execution, so exact historical versions remain unrecovered.
