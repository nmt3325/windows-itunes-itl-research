...
`Library` verifies both outer hdfm+0x30 and inner mfdh+0x30 against the parsed section count. For the observed record sizes, album PIDs, artist PIDs and playlist local IDs must each be nonzero and unique within their own namespace. Nonzero secondary track IDs must be unique. The four exact native-qualified template-free fixtures carry zero at `mith+0x1f4`, so read/no-op paths preserve zero; semantic and structural edits retain the stricter observed write gate that requires every modeled secondary ID to be nonzero. Item local IDs and PIDs must be nonzero and unique within each playlist. These are not a global string-atom namespace or a rule equating current item IDs with order tokens. Ambiguous same-PID auxiliary imports are refused before selecting any record, including equivalent-first/conflicting-later duplicates. The lower-level `Container` remains the opaque forensic route for other invalid semantic identities. The bounded distinction is reproduced by [`evidence/research/20260925/trailer-coverage-audit/report.json`](evidence/research/20260925/trailer-coverage-audit/report.json); it is not a universal native-ID rule.
...
The [bounded 2026-09-25 Linux filesystem audit](evidence/research/20260925/filesystem-threat-model/README.md) exercises 21 normalized scenarios without changing production code. Stable-parent destination aliases/races and the listed injected failure boundaries behaved as above. Deterministically scheduled parent rename/swap and parent-symlink retarget witnesses confirm that pathname stability is a real precondition: a displaced complete temporary can remain, and a replacement parent that spoofs its basename can cause spoof bytes to be linked. The audit did not crash a process or cut power and therefore does not strengthen the hostile-directory or durability claims.

The [bounded Windows/NTFS follow-up](evidence/research/20260925/windows-filesystem-publication/README.md) executes 25 normalized scenarios on one Windows Server 2025 / CPython 3.12.10 / runner-temporary NTFS qualification: 9 direct, 6 scheduler-assisted real-operation, 8 injected-boundary, and 2 parent-triggered/reaped Win32 `TerminateProcess` cases, with zero unavailable. It corroborates stable-parent alias/race behavior and records the same pathname-instability limit for real parent directory, symlink, and junction replacement. Termination before link left one complete temporary and no destination; termination after the native link but before cleanup left a complete destination plus its temporary hard-link alias. The run performed zero power-loss and zero native-iTunes operations. It does not establish universal Windows/NTFS behavior, hostile-directory safety, network-filesystem behavior, universal atomicity, or directory-entry durability; U-17 remains open.
