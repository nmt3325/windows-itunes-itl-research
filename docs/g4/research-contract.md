...
The absolute task deadline is 2026-09-11 09:00 Asia/Tokyo. Each runner lease can end earlier. Checkpoint work frequently and follow coordinator freeze/save/cleanup windows; missing evidence remains missing. Reports distinguish actual execution from plans, original bytes from reconstructed text, and primary testcases from subtests. Finish only on verified deliverables, not a session-completed label.
