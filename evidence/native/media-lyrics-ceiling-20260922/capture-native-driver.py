...
def wait_ready(proc,log,timeout_seconds=45):
    ...
    while time.monotonic()<limit:
        ...
        time.sleep(.25)
    log.append({'timeout_seconds':timeout_seconds,'timeout_windows':snapshot()});raise TimeoutError(f'iTunes main UI not ready in {timeout_seconds:g}s')
