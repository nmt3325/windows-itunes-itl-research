...
root=Path(r'D:\a\_temp\gha-mcp\WINDOWS_RESEARCH_RUN\work\itl');r=root/'reports/static';out=r/'phase3'
...
targets=[0xec7b80,0xeb9b10];assert len(targets)<=4
...
(out/'targets-final-two.txt').write_text(''.join(f'{a:x}\n' for a in targets),encoding='ascii')
(out/'function-atlas-final-two.json').write_text(json.dumps({'exe_sha256':sha,'functions':atlas},indent=2),encoding='utf-8')
