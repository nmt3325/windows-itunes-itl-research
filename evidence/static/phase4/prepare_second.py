...
(O/'asm').mkdir(exist_ok=True);targets=[0xfa5b70,0xf690b0,0xf6e110,0xf930a0];atlas=[]
for rv in targets:
...
(O/'targets-second.txt').write_text('\n'.join(f'{x:x}' for x in targets)+'\n',encoding='utf-8')
(O/'atlas-second.json').write_text(json.dumps(atlas,indent=2),encoding='utf-8')
