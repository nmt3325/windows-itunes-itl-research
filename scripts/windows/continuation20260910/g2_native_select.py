"""NEW G2 picker fork. Original native_select.py is preserved byte-for-byte.
G2 authorization and deadline replace the G1 cutoff, without changing the clock.
...
def executable_preflight(path):
    """Read-only EXE pin; the ITL/media limits remain unchanged."""
    return g.executable_pin(path, g.EXE_SHA, expected_bytes=g.EXE_BYTES)

def select(destination, mode, output_dir, *, authorization):
    authorization.check(budget_seconds=90)
    ...
    try:
        n.require_stopped()
        cutoff = authorization.check(budget_seconds=90)
        if datetime.datetime.now(datetime.timezone.utc) >= cutoff:
            raise ValueError('Native cutoff')
        r['exe_pin'] = executable_preflight(n.EXE)
        g.need(r['exe_pin']['sha256'] == n.PIN, 'exe_pin_changed')
        ...
        authorization.check(budget_seconds=60)
