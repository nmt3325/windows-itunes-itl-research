...
def _possible_external_ref(library, excluded: Node, persistent_id: int, *,
                           replacing_members=False, identity_tag=b'miph') -> bool:
    return possible_reference(library, persistent_id, excluded=excluded,
                              replacing_members=replacing_members, identity_tag=identity_tag)
...
def _replace(library, playlist: Playlist, track_persistent_ids, allocator: Allocator) -> None:
    ...
    for item in playlist.items:
        if _possible_external_ref(library, playlist.node, uint(item.header, 68, 8),
                                  replacing_members=True, identity_tag=b'mtph'):
