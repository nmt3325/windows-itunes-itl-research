...
The 24 Smart Playlist attempts, both original field-matrix failures, the harness-only first media attempt, and both corrected media-backed mutation failures remain in the aggregate corpus and delivery inventory. A parsed or native-saved failure is still a failure, while a pre-mutation harness failure is not relabeled as product rejection. No row above closes U-01–U-18 or changes `independent_reimplementation_passed: false`.
