...
The integrated offline regression is 1048 passed / 5 skipped when run with the frozen native fixture roots documented below. This count is not a native-interoperability percentage.
...
The 24 Smart Playlist attempts, both original field-matrix failures, the harness-only first media attempt, three exact WAV-backed mutation failures, and the one exact MP3-backed mutation/restart pass remain in the aggregate corpus and delivery inventory. A parsed or native-saved failure is still a failure, while a pre-mutation harness failure is not relabeled as product rejection. The MP3 pass is media/interface-dependent and establishes neither a long-string boundary nor generic Lyrics support. No row above closes U-01–U-18 or changes `independent_reimplementation_passed: false`.
