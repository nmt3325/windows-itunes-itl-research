...
| Exact short-ASCII, Unicode, and 65,536-character rewrites are strict ID3v2.2 `ULT` prefixes whose retained MPEG tails are byte-identical to their complete original MP3s | O/N2 for three exact pairs | [`scripts/research/analyze_mp3_rewrite.py`](scripts/research/analyze_mp3_rewrite.py); [`tests/test_mp3_rewrite_analysis.py`](tests/test_mp3_rewrite_analysis.py) | retained formal reports in the short, boundary, and extended evidence directories; exact tag shapes only, not a generic media writer |
| One Unicode value and ASCII lengths through 65,536 survived exact mutation and restart on fresh deterministic MP3 tracks | N2 for 11 exact cases | [`scripts/windows/native_media_field_followup.py`](scripts/windows/native_media_field_followup.py); repeated-field matrix validation and strict ID3 tests | [`evidence/native/media-lyrics-boundary-20260922/README.md`](evidence/native/media-lyrics-boundary-20260922/README.md) and [`evidence/native/media-lyrics-extended-boundary-20260922/README.md`](evidence/native/media-lyrics-extended-boundary-20260922/README.md); 11/11 passed, but 65,536 is only a lower accepted point |
...
The 24 Smart Playlist attempts, both original field-matrix failures, the harness-only first media attempt, three exact WAV-backed mutation failures, 12 exact MP3-backed mutation/restart passes, and three reset-ITL authority probes remain in the aggregate corpus and delivery inventory. A parsed or native-saved failure is still a failure, while a pre-mutation harness failure is not relabeled as product rejection. The MP3 passes are media/interface-dependent; 65,536 characters is a lower accepted point, not an upper boundary, and the authority probes do not exclude hidden caches. No row above closes U-01–U-18 or changes `independent_reimplementation_passed: false`.
