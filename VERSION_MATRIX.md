...
| --- | --- | --- | --- | --- | --- | --- |
| Standalone Windows iTunes 12.13.10.3, x64, observed LE profile | Implemented and offline-tested; native flag variants observed | Implemented for modeled sections/records | Guarded 756-byte `mith` subset, including bounded Name/Unplayed behavior | Guarded ordinary playlists and closed local-WAV track operations | Extensive exact-candidate evidence; four exact template-free one-track/three-track raw/zlib fixtures also pass two isolated cycles | **Primary supported research profile** |
| Standalone Windows iTunes 12.13.11.1, x64, executable SHA-256 `c69e719c…dd73c` | Four exact inherited 12.13.10.3 raw/zlib inputs opened; all native saves independently parse as 12.13.11.1 | Upgrade/open/save parsing qualified only for the retained exact cohort; no general profile admission | Unqualified; no arbitrary 12.13.11.1 scalar/text write claim | Unqualified; no arbitrary 12.13.11.1 structural write claim | Four exact inputs passed eight strict cycles with normal exits and no fallback | **Exact-hash upgrade qualification only; not the primary supported profile** |
...
| Native COM `Lyrics` setter (not an `itlkit` write API) | Three exact WAV failures; 17 exact MP3 passes including one Unicode value and discrete tested ASCII points up to 16,777,209; nine path-specific native non-exact cases beginning at 16,777,210; no universal limit | Unqualified | Unqualified |
...
9. **Template-free reference fixtures:** [`evidence/native/reference-generated-20260922-passed/qualification-summary.json`](evidence/native/reference-generated-20260922-passed/qualification-summary.json) pins the raw/zlib one-track hashes; [`evidence/native/reference-multi-track-20260922-v2/qualification-summary.json`](evidence/native/reference-multi-track-20260922-v2/qualification-summary.json) pins the raw/zlib three-track hashes. Each of the four exact candidates passed two cycles with exact persistent identities, expected Unicode names, ordered membership, no fallback artifacts, normal exit, and independent parsing. The preceding UTF-8 preflight failure is separately retained and counts as zero native cycles. This qualifies only those hashes and does not extend generation support to other counts, values, 12.13.9.1, or another profile.
...
13. **Media-backed field mixed exact results:** [`evidence/native/media-field-followup-20260922-v2/summary.json`](evidence/native/media-field-followup-20260922-v2/summary.json) records two exact WAV mutation failures after successful initialization/baseline restart, while [`evidence/native/media-lyrics-ascii-20260922/summary.json`](evidence/native/media-lyrics-ascii-20260922/summary.json) adds the same nested Lyrics rejection for a short ASCII WAV case. The short deterministic MP3 case and 11 additional cases in [`evidence/native/media-lyrics-boundary-20260922/`](evidence/native/media-lyrics-boundary-20260922/) and [`evidence/native/media-lyrics-extended-boundary-20260922/`](evidence/native/media-lyrics-extended-boundary-20260922/) passed all four sessions, including one Unicode value and exact ASCII values through 65,536 characters. Strict analyses show ID3v2.2 `ULT` prefix rewrites with byte-identical retained MPEG tails for three representative cases. The preceding [`media-field-followup-20260922`](evidence/native/media-field-followup-20260922/README.md) run remains a pre-setter harness failure. These cohorts add neither another supported version nor generic Lyrics support/rejection.
14. **Bounded Lyrics media-tag precedence:** [`evidence/native/lyrics-authority-20260922/summary.json`](evidence/native/lyrics-authority-20260922/summary.json) records three reset-ITL probes on one retained track. Stable COM Lyrics followed original, stripped, and equal-length conflicting MP3 `ULT` inputs without changing media. This does not exclude hidden caches or establish a general storage-authority rule.
15. **Generated-ASCII Lyrics frame-size ceiling:** [`evidence/native/media-lyrics-ceiling-20260922/summary.json`](evidence/native/media-lyrics-ceiling-20260922/summary.json) records five exact restart-persisted cases through 16,777,209 characters and nine qualified native non-exact immediate readbacks beginning at the adjacent 16,777,210 request, with zero harness failures. Formal reports for the retained adjacent outputs prove a conforming maximum unsigned-24-bit ID3v2.2 `ULT` payload (`0xFFFFFF`) followed by a nonconformant modulo-wrapped payload (`0x000000`). This is one deterministic MP3/tag/COM/build path, not another supported version or a universal `Lyrics` limit.
16. **Adjacent-build exact-hash upgrade qualification:** [`evidence/research/20260927/itunes-12.13.11.1-version-qualification/qualification-summary.json`](evidence/research/20260927/itunes-12.13.11.1-version-qualification/qualification-summary.json) records four exact 12.13.10.3 input hashes and eight passing open/save/restart cycles on one signed standalone iTunes 12.13.11.1 executable hash. Every native save independently reports 12.13.11.1, and all cycles exited normally without fallback. The two fresh 12.13.10.3 modal stops were setup/update preflight failures before semantic qualification, not ITL rejections. This is upgrade compatibility for the exact cohort, not arbitrary 12.13.11.1 editing, another primary profile, independent reproduction, or U-01 closure.
